...
train_segformer.py
------------------
Row 3 of your benchmark — and the KEY NOVELTY vs Lucho et al. (2024), who
tested no transformer. SAME experiment, same clean split, only the model is
SegFormer-B2 (a Vision Transformer for segmentation).

    TRAIN: FUSeg + AZH
    VALID: FUSeg validation
    TEST : DFUC2022  (independent hospital -> generalization number)

SegFormer comes from segmentation_models_pytorch too, so the code stays almost
identical to the U-Net / DeepLabV3+ scripts. Encoder weights download from the
HuggingFace hub the first time (needs internet once).
...
# ============================ CONFIG: TRAINING ========================
IMG_SIZE   = 256
BATCH      = 8                     # if you get CUDA out-of-memory, lower to 4
EPOCHS     = 30
LR         = 6e-5                  # transformers like a smaller LR than CNNs
ENCODER    = "mit_b2"             # SegFormer-B2 backbone
...
def build_model():
    """SegFormer via smp. Falls back gracefully if weights can't download."""
    try:
        return smp.Segformer(encoder_name=ENCODER, encoder_weights="imagenet",
                             in_channels=3, classes=1).to(DEVICE)
    except Exception as e:
        print(f"[warn] could not load pretrained weights ({e}); training from scratch.")
        return smp.Segformer(encoder_name=ENCODER, encoder_weights=None,
                             in_channels=3, classes=1).to(DEVICE)
...
def main():
    ...
    print("\n" + "=" * 60)
    print("RESULTS — SegFormer-B2 (mit_b2)")
    print("=" * 60)
